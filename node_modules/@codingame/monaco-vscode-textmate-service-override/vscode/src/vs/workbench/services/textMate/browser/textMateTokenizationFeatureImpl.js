...
import { generateTokensCSSForColorMap, generateTokensCSSForFontMap } from '@codingame/monaco-vscode-api/vscode/vs/editor/common/languages/supports/tokenization';
...
let TextMateTokenizationFeature = class TextMateTokenizationFeature extends Disposable {
    ...
    startDebugMode(printFn, onStop) {
        if (this._debugMode) {
            this._notificationService.error(( localize(14277, "Already Logging.")));
            return;
        }
        ...
        if (this._debugMode) {
            this._progressService.withProgress({
                ...
                buttons: [( localize(14278, "Stop"))]
            }, (progress) => {
                progress.report({
                    message: ( localize(14279, "Preparing to log TM Grammar parsing. Press Stop when finished."))
                });
                return this._getVSCodeOniguruma().then((vscodeOniguruma) => {
                    vscodeOniguruma.setDefaultDebugCall(true);
                    progress.report({
                        message: ( localize(14280, "Now logging TM Grammar parsing. Press Stop when finished."))
                    });
    ...
    async _getOrCreateGrammarFactory() {
        ...
        const [vscodeTextmate, vscodeOniguruma] = await Promise.all([import('@codingame/monaco-vscode-api/_virtual/main').then(function (n) { return n.main; }), this._getVSCodeOniguruma()]);
    ...
    _updateTheme(colorTheme, forceUpdate) {
        if (!forceUpdate && this._currentTheme && this._currentTokenColorMap && equalsTokenRules(this._currentTheme.settings, colorTheme.tokenColors)
            && equals(this._currentTokenColorMap, colorTheme.tokenColorMap) && this._currentTokenFontMap && equals(this._currentTokenFontMap, colorTheme.tokenFontMap)) {
            return;
        }
    ...
    _getVSCodeOniguruma() {
        if (!this._vscodeOniguruma) {
            this._vscodeOniguruma = (async () => {
                const [vscodeOniguruma, wasm] = await Promise.all([import('../../../../../../../_virtual/main2.js').then(function (n) { return n.main; }), this._loadVSCodeOnigurumaWASM()]);
    ...
    async _loadVSCodeOnigurumaWASM() {
        if (isWeb) {
            const response = await fetch(( new URL('../../../../../../../external/vscode-oniguruma/release/onig.wasm', import.meta.url)).href);
