...
let VisualizedVariableRenderer = class VisualizedVariableRenderer extends AbstractExpressionsRenderer {
    ...
    getInputBoxOptions(expression) {
        ...
        return {
            ...
            ariaLabel: ( localize(7014, "Type new variable value")),
            validationOptions: {
    ...
    renderActionBar(actionBar, expression, _data) {
        ...
        if (viz.original) {
            const action = toAction({
                id: 'debugViz', label: ( localize(7015, 'Remove Visualizer')), class: ThemeIcon.asClassName(Codicon.eye), run: () => this.debugService.getViewModel().setVisualizedExpression(viz.original, undefined)
            });
            action.checked = true;
            primary.push(action);
    ...
    renderActionBar(actionBar, expression, data) {
        ...
        this.visualization.getApplicableFor(expression, cts.token).then(result => {
            ...
            if (actions.length === 0) ;
                ...
                actionBar.push(toAction({ id: 'debugViz', label: ( localize(7016, 'Visualize Variable...')), class: ThemeIcon.asClassName(Codicon.eye), run: () => this.pickVisualizer(actions, originalExpression, data) }), { icon: true, label: false });
            }
        });
    }
...
class VariablesAccessibilityProvider {
    getWidgetAriaLabel() {
        return localize(7017, "Debug Variables");
    }
    getAriaLabel(element) {
        if (element instanceof Scope) {
            return localize(7018, "Scope {0}", element.name);
        }
        if (element instanceof Variable) {
            return localize(7019, "{0}, value {1}", element.name, element.value);
        }
        return null;
    }
}
...
async function tryInstallHexEditor(extensionsWorkbenchService, notificationService) {
    try {
        await extensionsWorkbenchService.install(HEX_EDITOR_EXTENSION_ID, {
            justification: ( localize(7020, "Inspecting binary data requires this extension.")),
            enable: true
...
registerAction2(class extends ViewAction {
    constructor() {
        super({
            ...
            title: ( localize(7021, "Collapse All")),
            f1: false,
