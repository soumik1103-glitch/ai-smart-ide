...
let AnythingQuickAccessProvider = class AnythingQuickAccessProvider extends PickerQuickAccessProvider {
    ...
    static { this.NO_RESULTS_PICK = {
        label: ( localize(10658, "No matching results"))
    }; }
    ...
    doGetPicks(filter, options, disposables, token) {
        ...
        if (this.pickState.isQuickNavigating) {
            if (picks.length > 0) {
                picks.push({ type: 'separator', label: ( localize(10659, "recently opened")) });
            }
        ...
        return {
            ...
            additionalPicks: (async () => {
                ...
                return additionalPicks.length > 0 ? [
                    { type: 'separator', label: this.configuration.includeSymbols ? ( localize(10660, "file and symbol results")) : ( localize(10661, "file results")) },
                    ...additionalPicks
                ] : [];
            })(),
    ...
    getHelpPicks(query, token, runOptions) {
        ...
        const providers = this.lazyRegistry.value.getQuickAccessProviders()
            ...
            return {
                ...
                ariaLabel: ( localize(10662, "{0}, {1}", label, helpEntry.description)),
                accept: () => {
        ...
        if (this.quickChatService.enabled) {
            providers.push({
                label: ( localize(10663, "Open Quick Chat")),
    ...
    createAnythingPick(resourceOrEditor, configuration) {
        ...
        return {
            resource,
            label,
            ariaLabel: isDirty ? ( localize(10667, "{0} unsaved changes", labelAndDescription)) : labelAndDescription,
            description,
