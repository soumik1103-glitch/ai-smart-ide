...
let ExtensionsListView = class ExtensionsListView extends AbstractExtensionsListView {
    ...
    async queryGallery(query, options, token) {
        ...
        try {
            ...
            if (localExtensions.length) {
                ...
                return { model: ( new PagedModel(localExtensions)), disposables: ( new DisposableStore()), message: { text: ( localize(7754, "{0} Showing local extensions.", message.text)), severity: message.severity } };
            }
            throw error;
        }
    }
        ...
        if (this.bodyTemplate) {
            ...
            if (this.isBodyVisible()) {
                if (message) {
                    ...
                    this.bodyTemplate.messageBox.textContent = ( localize(7755, "No extensions found."));
                }
    ...
    getMessage(error) {
        if (this.isOfflineError(error)) {
            ...
            return { text: ( localize(7757, "Error while fetching extensions. {0}", getErrorMessage(error))), severity: Severity$1.Error };
        }
    }
    ...
    async installWorkspaceRecommendations() {
        ...
        if (installableRecommendations.length) {
            ...
            this.notificationService.notify({
                ...
                message: ( localize(7758, "There are no extensions to install."))
            });
        }
    }
}
