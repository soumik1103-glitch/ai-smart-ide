            ...
            return uploadLimiter.queue(async () => {
                ...
                if (target && entry.name && target.getChild(entry.name)) {
                    ...
                    await this.explorerService.applyBulkEdit([( new ResourceFileEdit(
                        ...
                        undoLabel: ( localize(8078, "Overwrite {0}", entry.name)),
                        progressLabel: ( localize(8079, "Overwriting {0}", entry.name)),
                    });
                    if (token.isCancellationRequested) {
                        return;
                    }
    ...
    async doImport(target, source, targetWindow, token) {
        ...
        if (folders.length > 0 && target.isRoot) {
            ...
            const buttons = [
                {
                    label: folders.length > 1 ?
                        ( localize(8083, "&&Copy Folders")) :
                        ( localize(8084, "&&Copy Folder")),
            ...
            if (( folders.some(folder => workspaceFolderSchemas.indexOf(folder.uri.scheme) >= 0))) {
                buttons.unshift({
                    label: folders.length > 1 ?
                        ( localize(8085, "&&Add Folders to Workspace")) :
                        ( localize(8086, "&&Add Folder to Workspace")),
                ...
                message = folders.length > 1 ?
                    ( localize(8089, "Are you sure to want to copy folders?")) :
                    ( localize(8090, "Are you sure to want to copy '{0}'?", basename(folders[0].uri)));
            }
    ...
    async importResources(target, resources, token) {
        if (resources && resources.length > 0) {
            ...
            if (inaccessibleFileCount > 0) {
                this.notificationService.error(inaccessibleFileCount > 1 ? ( localize(8091, "Some or all of the dropped files could not be accessed for import.")) : ( localize(8092, "The dropped file could not be accessed for import.")));
            }
            ...
            await this.explorerService.applyBulkEdit(resourceFileEdits, {
                undoLabel: resourcesFiltered.length === 1 ?
                    ( localize(8093, "Import {0}", basename(resourcesFiltered[0]))) :
                    ( localize(8094, "Import {0} resources", resourcesFiltered.length)),
                progressLabel: resourcesFiltered.length === 1 ?
                    ( localize(8095, "Copying {0}", basename(resourcesFiltered[0]))) :
                    ( localize(8096, "Copying {0} resources", resourcesFiltered.length)),
...
let FileDownload = class FileDownload {
    ...
    download(source) {
        ...
        const downloadPromise = this.progressService.withProgress({
            ...
            delay: 800,
            cancellable: isWeb,
            title: ( localize(8097, "Downloading"))
    ...
    async doDownloadNative(explorerItem, progress, cts) {
        ...
        const destination = await this.fileDialogService.showSaveDialog({
            ...
            saveLabel: ( localize(8100, "Download")),
            title: ( localize(8101, "Choose Where to Download")),
            defaultUri
        });
        if (destination) {
            ...
            await this.explorerService.applyBulkEdit([( new ResourceFileEdit(explorerItem.resource, destination, { overwrite: true, copy: true }))], {
                undoLabel: ( localize(8102, "Download {0}", explorerItem.name)),
                progressLabel: ( localize(8103, "Downloading {0}", explorerItem.name)),
                progressLocation: ProgressLocation.Window
            });
        }
        else {
...
function getFileOverwriteConfirm(name) {
    return {
        ...
        detail: ( localize(8105, "This action is irreversible!")),
        primaryButton: ( localize(8106, "&&Replace")),
        type: 'warning'
    };
}
function getMultipleFilesOverwriteConfirm(files) {
    if (files.length > 1) {
        return {
            ...
            detail: getFileNamesMessage(files) + '\n' + ( localize(8105, "This action is irreversible!")),
            primaryButton: ( localize(8106, "&&Replace")),
            type: 'warning'
        };
    }
