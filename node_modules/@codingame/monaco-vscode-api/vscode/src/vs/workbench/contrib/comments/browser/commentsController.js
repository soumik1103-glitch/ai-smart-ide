    ...
    _findNearestCommentingRange(reverse) {
        ...
        if (this.accessibilityService.isScreenReaderOptimized()) {
            ...
            if (commentRangeStart && commentRangeEnd) {
                ...
                oneLine ? status(( localize(6350, "Line {0}", commentRangeStart))) : status(( localize(6351, "Lines {0} to {1}", commentRangeStart, commentRangeEnd)));
            }
        }
    }
    nextCommentingRange() {
            ...
            if (!this._hasRespondedToEditorChange) {
                if (( this._commentInfos.some(
                    ...
                    if (verbose) {
                        ...
                        status(( localize(6354, "Editor has commenting ranges.")));
                    }
                }
            }
        });
    ...
    addCommentAtLine(range, e) {
        ...
        if (!newCommentInfos.length || !this.editor?.hasModel()) {
            ...
            if (!newCommentInfos.length) {
                if (range) {
                    this.notificationService.error(( localize(6355, "The cursor must be within a commenting range to add a comment.")));
                }
                else {
                    this.notificationService.error(( localize(6356, "File comments are not allowed on this file.")));
                }
            }
            return Promise.resolve();
        }
        if (newCommentInfos.length > 1) {
            if (e && range) {
                ...
                return this.quickInputService.pick(picks, { placeHolder: ( localize(6357, "Select Comment Provider")), matchOnDescription: true }).then(pick => {
                    if (!pick) {
                        return;
                    }
