...
class PortColumn {
    constructor() {
        this.label = ( localize(10316, "Port"));
        this.tooltip = ( localize(10317, "The label and remote port number of the forwarded port."));
        this.weight = 1;
        this.templateId = 'actionbar';
    }
    project(row) {
...
class LocalAddressColumn {
    constructor() {
        this.label = ( localize(10318, "Forwarded Address"));
        this.tooltip = ( localize(10319, "The address that the forwarded port is available at."));
        this.weight = 1;
        this.templateId = 'actionbar';
    }
    project(row) {
    ...
    static getHoverText(localAddress) {
        return function (configurationService) {
            ...
            if (editorConf.multiCursorModifier === 'ctrlCmd') {
                if (isMacintosh) {
                    clickLabel = ( localize(10320, "option + click"));
                }
                else {
                    clickLabel = ( localize(10321, "alt + click"));
                }
            }
            else {
                if (isMacintosh) {
                    clickLabel = ( localize(10322, "cmd + click"));
                }
                else {
                    clickLabel = ( localize(10323, "ctrl + click"));
                }
            }
...
class RunningProcessColumn {
    constructor() {
        this.label = ( localize(10324, "Running Process"));
        this.tooltip = ( localize(10325, "The command line of the process that is using the port."));
        this.weight = 2;
        this.templateId = 'actionbar';
    }
    project(row) {
...
class OriginColumn {
    constructor() {
        this.label = ( localize(10326, "Origin"));
...
class PrivacyColumn {
    constructor() {
        this.label = ( localize(10328, "Visibility"));
        this.tooltip = ( localize(10329, "The availability of the forwarded port."));
        this.weight = 1;
        this.templateId = 'actionbar';
    }
    project(row) {
...
let ActionBarRenderer = class ActionBarRenderer extends Disposable {
    ...
    renderInputBox(container, editableData) {
        ...
        const inputBox = ( new InputBox(container, this.contextViewService, {
            ariaLabel: ( localize(10330, "Press Enter to confirm or Escape to cancel.")),
            validationOptions: {
                validation: (value) => {
...
class TunnelItem {
    ...
    get processDescription() {
        ...
        if (this.runningProcess) {
            ...
            description = ( localize(10331, "Process information unavailable"));
        }
        return description;
    }
    get tooltipPostfix() {
    ...
    get iconTooltip() {
        ...
        if (!isAdd) {
            return `${this.processDescription ? ( localize(10334, "Port has running process.")) :
                ( localize(10335, "No running process."))}`;
        }
        else {
            return this.label;
        }
    ...
    get portTooltip() {
        const isAdd = this.tunnelType === TunnelType.Add;
        if (!isAdd) {
            return `${this.name ? ( localize(10336, "Port labeled {0}. ", this.name)) : ''}`;
        }
        else {
            return '';
        }
...
const TunnelViewFocusContextKey = ( new RawContextKey('tunnelViewFocus', false, ( localize(10338, "Whether the Ports view has focus."))));
...
let TunnelPanel = class TunnelPanel extends ViewPane {
    ...
    static { this.TITLE = ( localize2(10339, "Ports")); }
    ...
    createTable() {
        ...
        this.table = this.instantiationService.createInstance(WorkbenchTable, 'RemoteTunnels', widgetContainer, ( new TunnelTreeVirtualDelegate(this.remoteExplorerService)), columns, [actionBarRenderer], {
            ...
            accessibilityProvider: {
                ...
                getWidgetAriaLabel: () => ( localize(10340, "Tunnel View"))
            },
...
(function (LabelTunnelAction) {
    ...
    LabelTunnelAction.LABEL = ( localize(10341, "Set Port Label"));
            ...
            if (tunnelContext) {
                ...
                return (new Promise(resolve => {
                    ...
                    remoteExplorerService.setEditable(tunnelItem, TunnelEditId.Label, {
                        ...
                        placeholder: ( localize(10342, "Port label")),
                        startingValue
                    });
                }));
            }
...
const invalidPortString = ( localize(10343, "Forwarded port should be a number or a host:port."));
const maxPortNumber = 65536;
const invalidPortNumberString = ( localize(10344, "Port number must be \u2265 0 and < {0}.", maxPortNumber));
const requiresSudoString = ( localize(10345, "May Require Sudo"));
const alreadyForwarded = ( localize(10346, "Port is already forwarded"));
...
(function (ForwardPortAction) {
    ...
    ForwardPortAction.LABEL = ( localize2(10347, "Forward a Port"));
    ForwardPortAction.TREEITEM_LABEL = ( localize(10348, "Forward Port"));
    const forwardPrompt = ( localize(10349, "Port number or address (eg. 3000 or 10.10.10.10:2000)."));
    ...
    function error(notificationService, tunnelOrError, host, port) {
        if (!tunnelOrError) {
            ...
            notificationService.warn(( localize(10351, "Unable to forward {0}:{1}. {2}", host, port, tunnelOrError)));
        }
    }
...
(function (ClosePortAction) {
    ...
    ClosePortAction.LABEL = ( localize2(10353, "Stop Forwarding Port"));
    function inlineHandler() {
        return async (accessor, arg) => {
    ...
    function commandPaletteHandler() {
        return async (accessor) => {
            ...
            const result = await quickInputService.pick(picks, { placeHolder: ( localize(10354, "Choose a port to stop forwarding")) });
...
(function (OpenPortInBrowserAction) {
    ...
    OpenPortInBrowserAction.LABEL = ( localize(10355, "Open in Browser"));
    function handler() {
        return async (accessor, arg) => {
            let key;
            if (isITunnelItem(arg)) {
...
(function (OpenPortInPreviewAction) {
    ...
    OpenPortInPreviewAction.LABEL = ( localize(10356, "Preview in Editor"));
    function handler() {
        return async (accessor, arg) => {
            let key;
            if (isITunnelItem(arg)) {
...
(function (OpenPortInBrowserCommandPaletteAction) {
    ...
    OpenPortInBrowserCommandPaletteAction.LABEL = ( localize(10357, "Open Port in Browser"));
    function handler() {
        return async (accessor, arg) => {
            const remoteExplorerService = accessor.get(IRemoteExplorerService);
            const tunnelService = accessor.get(ITunnelService);
            ...
            if (options.length === 0) {
                options.push({
                    label: ( localize(10358, "No ports currently forwarded. Open the Ports view to get started."))
                });
            }
            else {
                options.push({
                    label: ( localize(10359, "Open the Ports view..."))
                });
            }
            const picked = await quickPickService.pick(options, { placeHolder: ( localize(10360, "Choose the port to open")) });
...
(function (CopyAddressAction) {
    ...
    CopyAddressAction.INLINE_LABEL = ( localize(10361, "Copy Local Address"));
    CopyAddressAction.COMMANDPALETTE_LABEL = ( localize(10362, "Copy Forwarded Port Address"));
    ...
    function commandPaletteHandler() {
        return async (accessor, arg) => {
            ...
            const result = await quickInputService.pick(makeTunnelPicks(tunnels, remoteExplorerService, tunnelService), { placeHolder: ( localize(10363, "Choose a forwarded port")) });
            if (result && result.tunnel) {
...
(function (ChangeLocalPortAction) {
    ...
    ChangeLocalPortAction.LABEL = ( localize(10364, "Change Local Address Port"));
    function validateInput(tunnelService, value, canElevate) {
        if (!value.match(/^[0-9]+$/)) {
            return { content: ( localize(10365, "Local port should be a number.")), severity: Severity$1.Error };
        }
        ...
        return async (accessor, arg) => {
            ...
            if (tunnelContext) {
                ...
                remoteExplorerService.setEditable(tunnelItem, TunnelEditId.LocalPort, {
                    ...
                    placeholder: ( localize(10367, "New local port"))
                });
            }
        };
    }
...
(function (SetTunnelProtocolAction) {
    ...
    SetTunnelProtocolAction.LABEL_HTTP = ( localize(10368, "HTTP"));
    SetTunnelProtocolAction.LABEL_HTTPS = ( localize(10369, "HTTPS"));
...
MenuRegistry.appendMenuItem(MenuId.TunnelContext, ({
    ...
    title: ( localize(10370, "Port Visibility")),
...
MenuRegistry.appendMenuItem(MenuId.TunnelContext, ({
    ...
    title: ( localize(10371, "Change Port Protocol")),
