...
class ExpressionContainer {
        ...
        try {
            ...
            const vars = ( response.body.variables.filter(v => !!v).map((v) => {
                ...
                return (new Variable(
                    ...
                    '',
                    undefined,
                    localize(7110, "Invalid variable attributes"),
                    0,
                    0,
                    undefined,
                    { kind: 'virtual' },
    ...
    async evaluateExpression(expression, session, stackFrame, context, keepLazyVars = false, location) {
        if (!session || (!stackFrame && context !== 'repl')) {
            this.value = context === 'repl' ? ( localize(7111, "Please start a debug session to evaluate expressions")) : Expression.DEFAULT_VALUE;
            this.reference = 0;
            return false;
        }
        this.session = session;
...
class Expression extends ExpressionContainer {
    static { this.DEFAULT_VALUE = ( localize(7112, "not available")); }
    ...
    get stateLabel() {
        if (this.stoppedDetails) {
            return this.stoppedDetails.description ||
                (this.stoppedDetails.reason ? ( localize(7113, "Paused on {0}", this.stoppedDetails.reason)) : ( localize(7114, "Paused")));
        }
        return localize(7115, "Running");
    }
