...
let InstallCountWidget = InstallCountWidget_1 = class InstallCountWidget extends ExtensionWidget {
    ...
    render() {
        ...
        if (!this.small) {
            this.disposables.add(this.hoverService.setupManagedHover(getDefaultHoverDelegate('mouse'), this.container, ( localize(7759, "Install count"))));
        }
    }
...
let RatingsWidget = class RatingsWidget extends ExtensionWidget {
    ...
    render() {
        ...
        if (this.small) {
            ...
            this.containerHover.update(( localize(7760, "Average rating: {0} out of 5", rating)));
...
let PublisherWidget = class PublisherWidget extends ExtensionWidget {
    ...
    render() {
        ...
        if (this.small) {
            ...
            this.containerHover = this.disposables.add(this.hoverService.setupManagedHover(getDefaultHoverDelegate('mouse'), this.element, ( localize(7761, "Publisher ({0})", this.extension.publisherDisplayName))));
            append(this.element, publisherDisplayName);
            if (this.extension.publisherDomain?.verified) {
                append(this.element, verifiedPublisher);
...
let SponsorWidget = class SponsorWidget extends ExtensionWidget {
    ...
    render() {
        ...
        const label = $('span', undefined, ( localize(7763, "Sponsor")));
...
let ExtensionKindIndicatorWidget = class ExtensionKindIndicatorWidget extends ExtensionWidget {
    ...
    render() {
        ...
        if (this.extension?.private) {
            ...
            if (!this.small) {
                append(this.element, $('span.private-extension-label', undefined, ( localize(7765, "Private Extension"))));
            }
            return;
        }
        if (!this.small) {
        ...
        if (workspaceFolder && this.extension.isWorkspaceScoped) {
            this.element.textContent = ( localize(7766, "Workspace Extension"));
            ...
            this.element.textContent = ( localize(7767, "Local Extension"));
        }
    }
};
...
let SyncIgnoredWidget = class SyncIgnoredWidget extends ExtensionWidget {
    ...
    render() {
        ...
        if (this.extension && this.extension.state === ExtensionState.Installed && this.userDataSyncEnablementService.isEnabled() && this.extensionsWorkbenchService.isExtensionIgnoredToSync(this.extension)) {
            ...
            this.disposables.add(this.hoverService.setupManagedHover(getDefaultHoverDelegate('mouse'), element, ( localize(7768, "This extension is ignored during sync."))));
    ...
    render() {
        ...
        if (this.extensionViewState.filters.featureId && this.extension.state === ExtensionState.Installed) {
            ...
            if (feature?.icon && accessData) {
                ...
                featureAccessTimeElement.textContent = ( localize(7769, "{0} reqs", accessData.accessTimes.length));
...
let ExtensionHoverWidget = ExtensionHoverWidget_1 = class ExtensionHoverWidget extends ExtensionWidget {
    ...
    getHoverMarkdown() {
        ...
        if (this.extension.private) {
            markdown.appendMarkdown(`$(${privateExtensionIcon.id}) ${( localize(7765, "Private Extension"))}`);
            addSeparator = true;
        }
        if (this.extension.state === ExtensionState.Installed) {
            ...
            if (this.extension.publisherSponsorLink) {
                ...
                markdown.appendMarkdown(`$(${sponsorIcon.id}) [${( localize(7763, "Sponsor"))}](${this.extension.publisherSponsorLink})`);
                addSeparator = true;
            }
        }
        if (addSeparator) {
        ...
        if (location) {
            if (this.extension.isWorkspaceScoped && this.contextService.isInsideWorkspace(location)) {
                markdown.appendMarkdown(( localize(7766, "Workspace Extension")));
            }
            else {
                markdown.appendMarkdown(( localize(7767, "Local Extension")));
            }
            markdown.appendText(`\n`);
        }
        ...
        if (this.extension.outdated) {
            markdown.appendMarkdown(( localize(7771, "Latest version:")));
        ...
        if (extensionRuntimeStatus || extensionFeaturesAccessData.size || extensionStatus.length || runtimeState || recommendationMessage || preReleaseMessage) {
            ...
            if (extensionRuntimeStatus) {
                if (extensionRuntimeStatus.activationTimes) {
                    ...
                    markdown.appendMarkdown(`${( localize(7772, "Activation time"))}${extensionRuntimeStatus.activationTimes.activationReason.startup ? ` (${( localize(7773, "Startup"))})` : ''}: \`${activationTime}ms\``);
                    markdown.appendText(`\n`);
                }
                if (extensionRuntimeStatus.runtimeErrors.length || extensionRuntimeStatus.messages.length) {
                    ...
                    const errorsLink = extensionRuntimeStatus.runtimeErrors.length ? `[${extensionRuntimeStatus.runtimeErrors.length === 1 ? ( localize(7774, '1 uncaught error')) : ( localize(7775, '{0} uncaught errors', extensionRuntimeStatus.runtimeErrors.length))}](${createCommandUri('extension.open', this.extension.identifier.id, ExtensionEditorTab.Features)})` : undefined;
                    const messageLink = extensionRuntimeStatus.messages.length ? `[${extensionRuntimeStatus.messages.length === 1 ? ( localize(7776, '1 message')) : ( localize(7777, '{0} messages', extensionRuntimeStatus.messages.length))}](${createCommandUri('extension.open', this.extension.identifier.id, ExtensionEditorTab.Features)})` : undefined;
            ...
            if (extensionFeaturesAccessData.size) {
                ...
                for (const [featureId, accessData] of extensionFeaturesAccessData) {
                    if (accessData?.accessTimes.length) {
                        ...
                        if (feature) {
                            markdown.appendMarkdown(( localize(7778, "{0} usage", feature.label)));
    ...
    static getPreReleaseMessage(extension) {
        ...
        const preReleaseVersionLink = `[${( localize(7780, "Pre-Release version"))}](${createCommandUri('workbench.extensions.action.showPreReleaseVersion', extension.identifier.id)})`;
        return localize(7781, "This extension has a {0} available", preReleaseVersionLink);
    }
};
...
let ExtensionRecommendationWidget = class ExtensionRecommendationWidget extends ExtensionWidget {
    ...
    getRecommendationStatus() {
        ...
        if (extRecommendations[this.extension.identifier.id.toLowerCase()]) {
            ...
            return { icon: undefined, message: ( localize(7782, "You have chosen not to receive recommendations for this extension.")) };
        }
        return undefined;
    }
};
...
const extensionRatingIconColor = registerColor('extensionIcon.starForeground', { light: '#DF6100', dark: '#FF8E00', hcDark: '#FF8E00', hcLight: textLinkForeground }, ( localize(7783, "The icon color for extension ratings.")), false);
const extensionPreReleaseIconColor = registerColor('extensionIcon.preReleaseForeground', { dark: '#1d9271', light: '#1d9271', hcDark: '#1d9271', hcLight: textLinkForeground }, ( localize(7784, "The icon color for pre-release extension.")), false);
registerColor('extensionIcon.sponsorForeground', { light: '#B51E78', dark: '#D758B3', hcDark: null, hcLight: '#B51E78' }, ( localize(7785, "The icon color for extension sponsor.")), false);
const extensionPrivateBadgeBackground = registerColor('extensionIcon.privateForeground', { dark: '#ffffff60', light: '#00000060', hcDark: '#ffffff60', hcLight: '#00000060' }, ( localize(7786, "The icon color for private extensions.")));
