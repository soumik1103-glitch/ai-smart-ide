        ...
        if (this.gallery) {
            ...
            if (!cache) {
                if (this.gallery.assets.manifest) {
                    ...
                    this.logService.error(( localize(7787, "Manifest is not found")), this.identifier.id);
                }
            }
            return cache;
        }
...
let ExtensionsWorkbenchService = class ExtensionsWorkbenchService extends Disposable {
    ...
    async updateAutoUpdateForAllExtensions(isAutoUpdateEnabled) {
        ...
        const result = await this.dialogService.confirm({
            title: ( localize(7788, "Auto Update Extensions")),
            message: isAutoUpdateEnabled
                ? ( localize(7789, "Do you want to enable auto update for all extensions?"))
                : ( localize(7790, "Do you want to disable auto update for all extensions?")),
            detail: ( localize(
                7791,
    ...
    updateExtensionsPinnedState(pinned) {
        return this.progressService.withProgress({
            ...
            title: ( localize(7792, "Updating Extensions Auto Update State")),
    ...
    computeExtensionsNotifications() {
        ...
        if (invalidExtensions.length) {
            if (( invalidExtensions.some(e => e.local && e.local.manifest.engines?.vscode &&
                ...
                computedNotificiations.push({
                    message: ( localize(7796, "Invalid extensions detected. Review them.")),
                    severity: Severity$1.Warning,
                    extensions: invalidExtensions,
    ...
    async updateRunningExtensions(message = ( localize(7798, "Changing extension enablement")), auto = false) {
        const toAdd = [];
        const toRemove = [];
            ...
            if (await this.extensionService.stopExtensionHosts(message, auto)) {
                ...
                if (auto) {
                    this.notificationService.notify({
                        ...
                        message: ( localize(7799, "Extensions were auto restarted to enable updates.")),
    ...
    getRuntimeState(extension) {
        ...
        const reloadActionLabel = reloadAction === ExtensionRuntimeActionType.ReloadWindow ? ( localize(7800, "reload window")) : ( localize(7801, "restart extensions"));
        if (isUninstalled) {
                ...
                if (isEnabled) {
                    ...
                    if (isSameExtensionRunning) {
                        if (!runningExtension.isUnderDevelopment && (extension.version !== runningExtension.version || extension.local.targetPlatform !== runningExtension.targetPlatform)) {
                            ...
                            return { action: reloadAction, reason: ( localize(7806, "Please {0} to enable the updated extension.", reloadActionLabel)) };
                        }
                        if (this.extensionsServers.length > 1) {
                            ...
                            if (extensionInOtherServer) {
                                if (runningExtensionServer === this.extensionManagementServerService.remoteExtensionManagementServer && this.extensionManifestPropertiesService.prefersExecuteOnUI(extension.local.manifest) && extensionInOtherServer.server === this.extensionManagementServerService.localExtensionManagementServer) {
                                    return { action: reloadAction, reason: ( localize(7807, "Please {0} to enable this extension locally.", reloadActionLabel)) };
                                }
                        ...
                        if (extension.server === this.extensionManagementServerService.localExtensionManagementServer && runningExtensionServer === this.extensionManagementServerService.remoteExtensionManagementServer) {
                            if (this.extensionManifestPropertiesService.prefersExecuteOnUI(extension.local.manifest)) {
                                return { action: reloadAction, reason: ( localize(7809, "Please {0} to enable this extension.", reloadActionLabel)) };
                            }
                        }
                    ...
                    if (isSameExtensionRunning) {
                        return { action: reloadAction, reason: ( localize(7810, "Please {0} to disable this extension.", reloadActionLabel)) };
                    }
                }
                return undefined;
            }
    ...
    async downloadVSIX(extensionId, versionKind) {
        ...
        if (!galleryExtension) {
            throw ( new Error(( localize(7811, "Extension '{0}' not found.", extensionId))));
        }
        ...
        for (const targetPlatform of version?.targetPlatforms ?? galleryExtension.allTargetPlatforms) {
            if (targetPlatform !== TargetPlatform.UNKNOWN && targetPlatform !== TargetPlatform.UNIVERSAL) {
                options.push({
                    label: targetPlatform === TargetPlatform.UNDEFINED ? ( localize(7812, "All Platforms")) : TargetPlatformToString(targetPlatform),
                    id: targetPlatform
                });
            }
        }
        if (options.length > 1) {
            const message = ( localize(7813, "Please select the platform for which you want to download the VSIX"));
        ...
        const result = await this.fileDialogService.showOpenDialog({
            title: ( localize(7814, "Select folder to download the VSIX")),
            canSelectFiles: false,
            canSelectFolders: true,
            canSelectMany: false,
            openLabel: ( localize(7815, "Download")),
        });
        if (!result?.[0]) {
            return;
        }
        this.progressService.withProgress({ location: ProgressLocation.Notification }, async (progress) => {
            try {
                progress.report({ message: ( localize(7816, "Downloading VSIX...")) });
                ...
                this.notificationService.info(( localize(7817, "Successfully downloaded the VSIX")));
            }
            catch (error) {
                this.notificationService.error(( localize(7818, "Error while downloading the VSIX: {0}", getErrorMessage(error))));
            }
        });
    }
    async pickVersionToDownload(extensionId) {
        ...
        if (!allVersions.length) {
            await this.dialogService.info(( localize(7819, "This extension has no other versions.")));
            return;
        }
        const picks = ( allVersions.map((v, i) => {
            return {
                id: v.version,
                label: v.version,
                description: `${fromNow(( new Date(Date.parse(v.date))), true)}${v.isPreReleaseVersion ? ` (${( localize(7820, "pre-release"))})` : ''}`,
                ariaLabel: `${v.isPreReleaseVersion ? 'Pre-Release version' : 'Release version'} ${v.version}`,
                data: v,
            };
        }));
        const pick = await this.quickInputService.pick(picks, {
            placeHolder: ( localize(7821, "Select Version to Download")),
            matchOnDetail: true
        });
        return pick?.data;
    }
    ...
    async canInstall(extension) {
        if (!(extension instanceof Extension)) {
            return ( new MarkdownString()).appendText(( localize(7824, "The provided object is not an extension.")));
        }
        if (extension.isMalicious) {
            return ( new MarkdownString()).appendText(( localize(7825, "This extension is reported to be problematic.")));
        }
        if (extension.deprecationInfo?.disallowInstall) {
            return ( new MarkdownString()).appendText(( localize(7826, "This extension is disallowed to be installed.")));
        }
        if (extension.gallery) {
            if (!extension.gallery.isSigned && shouldRequireRepositorySignatureFor(extension.private, await this.extensionGalleryManifestService.getExtensionGalleryManifest())) {
                return ( new MarkdownString()).appendText(( localize(7827, "This extension is not signed.")));
            }
    ...
    async install(arg, installOptions = {}, progressLocation) {
        ...
        if (arg instanceof URI) {
            ...
            if (extension?.isMalicious) {
                throw ( new Error(( localize(7825, "This extension is reported to be problematic."))));
            }
            if (gallery) {
            ...
            if (installOptions.justification) {
                ...
                buttons.push({
                    label: isString(installOptions.justification) || !installOptions.justification.action
                        ? ( localize(7832, "&&Install Extension"))
                        : ( localize(7833, "&&Install Extension and {0}", installOptions.justification.action)), run: () => true
                });
                if (!extension) {
                    buttons.push({ label: ( localize(7834, "Open Extension")), run: () => { this.open(extension); return false; } });
                }
                const result = await this.dialogService.prompt({
                    title: ( localize(7835, "Install Extension")),
                    message: extension ? ( localize(
                        7836,
                        "Would you like to install '{0}' extension from '{1}'?",
                        extension.displayName,
                        extension.publisherDisplayName
                    )) : ( localize(7837, "Would you like to install the extension?")),
                    detail: isString(installOptions.justification) ? installOptions.justification : installOptions.justification.reason,
                    cancelButton: true,
                    buttons,
                    checkbox: syncCheck ? {
                        label: ( localize(7838, "Sync this extension")),
                        checked: true,
                    } : undefined,
                });
                if (!result.result) {
        ...
        if (!extension) {
            throw ( new Error(( localize(7839, "Unable to install extension"))));
        }
        if (installOptions.enable) {
            if (extension.enablementState === EnablementState.DisabledWorkspace || extension.enablementState === EnablementState.DisabledGlobally) {
                if (installOptions.justification) {
                    const result = await this.dialogService.confirm({
                        title: ( localize(7840, "Enable Extension")),
                        message: ( localize(7841, "Would you like to enable '{0}' extension?", extension.displayName)),
                        detail: isString(installOptions.justification) ? installOptions.justification : installOptions.justification.reason,
                        primaryButton: isString(installOptions.justification) ? ( localize(7842, "&&Enable Extension")) : ( localize(7843, "&&Enable Extension and {0}", installOptions.justification.action)),
                    });
                    if (!result.confirmed) {
                        throw ( new CancellationError());
                    }
    ...
    async uninstall(e) {
        ...
        if (extension.local.isApplicationScoped && this.userDataProfilesService.profiles.length > 1) {
            const { confirmed } = await this.dialogService.confirm({
                title: ( localize(7845, "Uninstall Extension")),
                ...
                primaryButton: ( localize(7847, "Uninstall (All Profiles)"))
            });
            if (!confirmed) {
                throw ( new CancellationError());
            }
        ...
        return this.withProgress({
            location: ProgressLocation.Extensions,
            title: ( localize(7850, 'Uninstalling extension...')),
    ...
    doInstall(extension, installTask, progressLocation) {
        const title = extension ? ( localize(7854, "Installing '{0}' extension...", extension.displayName)) : ( localize(7855, 'Installing extension...'));
        return this.withProgress({
