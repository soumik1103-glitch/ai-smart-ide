...
import { LocalChatSessionUri, chatSessionResourceToId } from '../../model/chatUri.js';
...
function createManageTodoListToolData(writeOnly, includeDescription = true) {
    ...
    return {
        ...
        displayName: ( localize(6070, 'Manage and track todo items for task planning')),
        userDescription: ( localize(6071, 'Manage and track todo items for task planning')),
...
let ManageTodoListTool = class ManageTodoListTool extends Disposable {
    ...
    async prepareToolInvocation(context, _token) {
        ...
        switch (operation) {
            ...
            case 'read': {
                message = ( localize(6072, "Read todo list"));
                break;
            }
        }
        ...
        return {
            pastTenseMessage: ( new MarkdownString(message ?? ( localize(6073, "Updated todo list")))),
            toolSpecificData: {
                kind: 'todoList',
    ...
    generatePastTenseMessage(currentTodos, newTodos) {
        if (currentTodos.length === 0) {
            return newTodos.length === 1
                ? ( localize(6074, "Created 1 todo"))
                : ( localize(6075, "Created {0} todos", newTodos.length));
        }
        ...
        if (addedTodos.length > 0) {
            return addedTodos.length === 1
                ? ( localize(6078, "Added 1 todo"))
                : ( localize(6079, "Added {0} todos", addedTodos.length));
        }
        return localize(6080, "Updated todo list");
    }
