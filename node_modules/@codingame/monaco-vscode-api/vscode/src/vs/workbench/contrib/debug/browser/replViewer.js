...
class ReplAccessibilityProvider {
    getWidgetAriaLabel() {
        return localize(6989, "Debug Console");
    }
    getAriaLabel(element) {
        if (element instanceof Variable) {
            return localize(6990, "Variable {0}, value {1}", element.name, element.value);
        }
        if (element instanceof ReplOutputElement || element instanceof ReplEvaluationInput || element instanceof ReplEvaluationResult) {
            return element.value + (element instanceof ReplOutputElement && element.count > 1 ? ( localize(6991, ", occurred {0} times", element.count)) : '');
        }
        if (element instanceof RawObjectReplElement) {
            return localize(6992, "Debug console variable {0}, value {1}", element.name, element.value);
        }
        if (element instanceof ReplGroup) {
            return localize(6993, "Debug console group {0}", element.name);
        }
        return '';
    }
}
