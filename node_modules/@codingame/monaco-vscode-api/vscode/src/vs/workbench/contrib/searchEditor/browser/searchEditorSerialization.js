        ...
        for (let i = 0; i < str.length; i++) {
            if (str[i] === '\\') {
                ...
                if (escaped === 'n') {
                    ...
                    throw Error(( localize(10949, "All backslashes in Query string must be escaped (\\\\)")));
                }
            }
            else {
                out += str[i];
...
const serializeSearchResultForEditor = (searchResult, rawIncludePattern, rawExcludePattern, contextLines, labelFormatter, sortOrder, limitHit) => {
    ...
    const filecount = searchResult.fileCount() > 1 ? ( localize(10950, "{0} files", searchResult.fileCount())) : ( localize(10951, "1 file"));
    const resultcount = searchResult.count() > 1 ? ( localize(10952, "{0} results", searchResult.count())) : ( localize(10953, "1 result"));
    const info = [
        searchResult.count()
            ? `${resultcount} - ${filecount}`
            : ( localize(10954, "No Results")),
    ];
    if (limitHit) {
        info.push(( localize(
            10955,
