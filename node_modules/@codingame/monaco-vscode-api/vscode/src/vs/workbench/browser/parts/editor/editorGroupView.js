...
let EditorGroupView = EditorGroupView_1 = class EditorGroupView extends Themable {
    ...
    createContainerToolbar() {
        ...
        const containerToolbar = this._register(( new ActionBar(toolbarContainer, {
            ariaLabel: ( localize(3407, "Empty editor group actions")),
    ...
    updateContainer() {
        if (this.isEmpty) {
            ...
            this.element.setAttribute('aria-label', ( localize(3408, "{0} (empty)", this.ariaLabel)));
        }
        else {
    ...
    get label() {
        if (this.groupsLabel) {
            return localize(3409, "{0}: Group {1}", this.groupsLabel, this._index + 1);
        }
        return localize(3410, "Group {0}", this._index + 1);
    }
    get ariaLabel() {
        if (this.groupsLabel) {
            return localize(3411, "{0}: Editor Group {1}", this.groupsLabel, this._index + 1);
        }
        return localize(3412, "Editor Group {0}", this._index + 1);
    }
    get disposed() {
    ...
    doMoveOrCopyEditorAcrossGroups(editor, target, openOptions, internalOptions) {
        ...
        if (!keepCopy || editor.hasCapability(EditorInputCapabilities.Singleton) ) {
            ...
            if (typeof canMoveVeto === 'string') {
                this.dialogService.error(canMoveVeto, ( localize(3413, "Try saving or reverting the editor first and then try again.")));
                return false;
            }
        }
