    ...
    openLinkOccurrence(occurrence, openToSide, fromUserGesture = false) {
        ...
        link.resolve(CancellationToken.None).then(uri => {
            ...
            if (messageOrError === 'invalid') {
                ...
                this.notificationService.warn(( localize(1383, 'Failed to open this link because its target is missing.')));
            }
            else {
                onUnexpectedError(err);
            }
...
function getHoverMessage(link, useMetaKey) {
    ...
    const label = link.tooltip
        ? link.tooltip
        : executeCmd
            ? ( localize(1384, 'Execute command'))
            : ( localize(1385, 'Follow link'));
    const kb = useMetaKey
        ? isMacintosh
            ? ( localize(1386, "cmd + click"))
            : ( localize(1387, "ctrl + click"))
        : isMacintosh
            ? ( localize(1388, "option + click"))
            : ( localize(1389, "alt + click"));
    if (link.url) {
        let nativeLabel = '';
        if (/^command:/i.test(( link.url.toString()))) {
            ...
            if (match) {
                ...
                nativeLabel = ( localize(1390, "Execute command {0}", commandId));
            }
        }
...
class OpenLinkAction extends EditorAction {
    constructor() {
        super({
            ...
            label: ( localize2(1391, "Open Link")),
