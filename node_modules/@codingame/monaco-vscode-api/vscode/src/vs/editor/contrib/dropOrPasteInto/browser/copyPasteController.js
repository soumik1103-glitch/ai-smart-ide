...
const pasteWidgetVisibleCtx = ( new RawContextKey('pasteWidgetVisible', false, ( localize(1018, "Whether the paste widget is showing"))));
...
let CopyPasteController = class CopyPasteController extends Disposable {
    ...
    constructor(editor, instantiationService, _logService, _bulkEditService, _clipboardService, _commandService, _configService, _languageFeaturesService, _quickInputService, _progressService) {
        ...
        this._postPasteWidgetManager = this._register(instantiationService.createInstance(PostEditWidgetManager, 'pasteIntoEditor', editor, pasteWidgetVisibleCtx, { id: changePasteTypeCommandId, label: ( localize(1019, "Show paste options...")) }, () => CopyPasteController_1._configureDefaultAction ? [CopyPasteController_1._configureDefaultAction] : []));
    }
    changePasteType() {
    ...
    showPasteAsNoEditMessage(selections, preference) {
        const kindLabel = 'only' in preference
            ...
            : 'preferences' in preference
                ? (preference.preferences.length ? ( preference.preferences.map(preference => preference.value)).join(', ') : ( localize(1020, "empty")))
                : preference.providerId;
        MessageController.get(this._editor)?.showMessage(( localize(1021, "No paste edits for '{0}' found", kindLabel)), selections[0].getStartPosition());
    }
    doPasteInline(allProviders, selections, dataTransfer, metadata, clipboardEvent) {
            ...
            try {
                ...
                if (editSession.edits.length) {
                    ...
                    return this._postPasteWidgetManager.applyEditAndShowIfNeeded(selections, { activeEditIndex: this.getInitialActiveEditIndex(model, editSession.edits), allEdits: editSession.edits }, canShowWidget, async (edit, resolveToken) => {
                        ...
                        const resolved = await this._pasteProgressManager.showWhile(selections[0].getEndPosition(), ( localize(1022, "Resolving paste edit for '{0}'. Click to cancel", edit.title)), raceCancellation(Promise.race([showP.p, resolveP]), resolveToken), {
                            cancel: () => showP.cancel()
                        }, 0);
                        if (resolved) {
        ...
        this._pasteProgressManager.showWhile(selections[0].getEndPosition(), ( localize(1023, "Running paste handlers. Click to cancel and do basic paste")), p, {
            cancel: async () => {
                p.cancel();
    ...
    showPasteAsPick(preference, allProviders, selections, dataTransfer, metadata) {
            ...
            try {
                ...
                if (preference) {
                    ...
                    const configureDefaultItem = {
                        ...
                        label: ( localize(1024, "Configure default paste action")),
                        edit: undefined,
                    };
                    const selected = await this._quickInputService.pick([
                        ...
                        placeHolder: ( localize(1025, "Select Paste Action")),
                    });
        ...
        this._progressService.withProgress({
            ...
            title: ( localize(1026, "Running paste handlers")),
        }, () => p);
    }
