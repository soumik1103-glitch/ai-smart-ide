...
let LightBulbWidget = class LightBulbWidget extends Disposable {
    ...
    _updateLightbulbTitle(autoFix, autoRun) {
        ...
        if (autoRun) {
            this.title = ( localize(980, "Run: {0}", this.state.actions.validActions[0].action.title));
        }
            ...
            this.title = ( localize(982, "Show Code Actions ({0})", this._quickFixKbLabel));
        }
        else if (!autoFix) {
            this.title = ( localize(983, "Show Code Actions"));
        }
    }
    set title(value) {
