...
function validateExtensionManifest(productVersion, productDate, extensionLocation, extensionManifest, extensionIsBuiltin, validateApiVersion) {
    ...
    if (typeof extensionManifest.publisher !== 'undefined' && typeof extensionManifest.publisher !== 'string') {
        validations.push([Severity$1.Error, ( localize(1885, "property publisher must be of type `string`."))]);
        return validations;
    }
    if (typeof extensionManifest.name !== 'string') {
        validations.push([Severity$1.Error, ( localize(1886, "property `{0}` is mandatory and must be of type `string`", 'name'))]);
        return validations;
    }
    ...
    if (typeof extensionManifest.main !== 'undefined') {
        if (typeof extensionManifest.main !== 'string') {
            validations.push([Severity$1.Error, ( localize(1894, "property `{0}` can be omitted or must be of type `string`", 'main'))]);
            return validations;
        }
        else {
    ...
    if (!semverExports.valid(extensionManifest.version)) {
        validations.push([Severity$1.Error, ( localize(1898, "Extension version is not semver compatible."))]);
        return validations;
    }
    const notices = [];
