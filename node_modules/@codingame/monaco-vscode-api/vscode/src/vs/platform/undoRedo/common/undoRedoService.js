...
let UndoRedoService = class UndoRedoService {
    ...
    async _confirmAndExecuteWorkspaceUndo(strResource, element, editStackSnapshot, undoConfirmed) {
        if (element.canSplit() && !this._isPartOfUndoGroup(element)) {
            ...
            const { result } = await this._dialogService.prompt({
                ...
                message: ( localize(2431, "Would you like to undo '{0}' across all files?", element.label)),
                buttons: [
                    {
                        label: ( localize(2432, "&&Undo in {0} Files", editStackSnapshot.editStacks.length)),
                        run: () => UndoChoice.All
                    },
                    {
                        label: ( localize(2433, "Undo this &&File")),
                        run: () => UndoChoice.This
                    }
                ],
                cancelButton: {
    ...
    async _confirmAndContinueUndo(strResource, sourceId, element) {
        const result = await this._dialogService.confirm({
            message: ( localize(2435, "Would you like to undo '{0}'?", element.label)),
            primaryButton: ( localize(2436, "&&Yes")),
            cancelButton: ( localize(2437, "No"))
        });
        if (!result.confirmed) {
            return;
        }
