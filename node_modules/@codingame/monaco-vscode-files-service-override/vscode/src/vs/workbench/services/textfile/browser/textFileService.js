...
let AbstractTextFileService = class AbstractTextFileService extends Disposable {
    ...
    static { this.TEXTFILE_SAVE_CREATE_SOURCE = SaveSourceRegistry.registerSource('textFileCreate.source', ( localize(14297, "File Created"))); }
    static { this.TEXTFILE_SAVE_REPLACE_SOURCE = SaveSourceRegistry.registerSource('textFileOverwrite.source', ( localize(14298, "File Replaced"))); }
    ...
    provideDecorations() {
        const provider = this._register(new (class extends Disposable {
            constructor(files) {
                super();
                this.files = files;
                this.label = ( localize(14299, "Text File Model Decorations"));
            ...
            provideDecorations(uri) {
                ...
                if (isReadonly && isOrphaned) {
                    return {
                        ...
                        strikethrough: true,
                        tooltip: ( localize(14300, "Deleted, Read-only")),
                    };
                }
                else if (isReadonly) {
                    return {
                        letter: Codicon.lockSmall,
                        tooltip: ( localize(14301, "Read-only")),
                    };
                }
                else if (isOrphaned) {
                    return {
                        color: listErrorForeground,
                        strikethrough: true,
                        tooltip: ( localize(14302, "Deleted")),
                    };
                }
                return undefined;
            }
    ...
    async doRead(resource, options) {
        ...
        try {
            ...
            if (error.decodeStreamErrorKind === DecodeStreamErrorKind.STREAM_IS_BINARY) {
                throw ( new TextFileOperationError(( localize(14303, "File seems to be binary and cannot be opened as text")), TextFileOperationResult.FILE_IS_BINARY, options));
            }
            else {
                throw error;
            }
    ...
    async confirmOverwrite(resource) {
        const { confirmed } = await this.dialogService.confirm({
            ...
            primaryButton: ( localize(14306, "&&Replace")),
        });
        return confirmed;
    }
    async confirmMakeWriteable(resource) {
        const { confirmed } = await this.dialogService.confirm({
            ...
            detail: ( localize(14308, "Paths can be configured as read-only via settings.")),
            primaryButton: ( localize(14309, "&&Save Anyway"))
        });
        return confirmed;
    }
